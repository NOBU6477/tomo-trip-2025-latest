...
(function() {
  ...
  function initCameraFix() {
    // カメラボタンを探す
    const buttons = document.querySelectorAll('.camera-button, .btn-primary[class*="camera"], button:has(i.bi-camera), button:has(.fa-camera)');
    ...
    // モーダル内のボタンを確認
    fixModalCameraButtons();
  }
  
  // モーダル内のカメラ関連ボタンを修正
  function fixModalCameraButtons() {
    // 撮影ボタンと写真使用ボタンを探す
    const captureButtons = document.querySelectorAll('.modal .btn-capture, .modal #capture-btn');
    const usePhotoButtons = document.querySelectorAll('.modal .btn-use-photo, .modal #use-photo-btn');
    ...
    allButtons.forEach(btn => {
      ...
      if (text.includes('撮影')) {
        if (!Array.from(captureButtons).includes(btn)) {
          captureButtons.push(btn);
        }
      } else if (text.includes('この写真を使用')) {
        if (!Array.from(usePhotoButtons).includes(btn)) {
          usePhotoButtons.push(btn);
        }
      }
    });
    ...
    // 写真使用ボタンの修正
    usePhotoButtons.forEach(button => {
      // すでに処理済みならスキップ
      if (button.getAttribute('data-fixed') === 'true') {
        return;
      }
