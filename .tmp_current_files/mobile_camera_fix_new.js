...
(function() {
  ...
  function initMobileCameraFix() {
    ...
    // カメラエラーイベントの発行関数をグローバルに設定
    window.dispatchCameraError = function(errorData) {
    ...
    // iOSのSafariでカメラアクセスの問題が起きた場合のイベントリスナー
    document.addEventListener('camera-error', function(event) {
      console.log('カメラエラーイベントを検知:', event.detail);
      // 既存のフォールバック処理を呼び出し
      if (window.cameraFallback && typeof window.cameraFallback.activateFallback === 'function') {
        window.cameraFallback.activateFallback();
      }
    });
  ...
  function hookIntoExistingCameraHandlers() {
    ...
    document.body.addEventListener('shown.bs.modal', function(event) {
      ...
      if (modal.id && modal.id.includes('camera') || modal.classList.contains('camera-modal')) {
        console.log('カメラモーダルが表示されました:', modal.id);
        setupCameraButtons(modal);
      }
    });
    
    // ドキュメント読み込み完了時にすでに表示されているモーダルをチェック
    document.querySelectorAll('.modal.show').forEach(function(modal) {
      if (modal.id && modal.id.includes('camera') || modal.classList.contains('camera-modal')) {
        console.log('すでに表示されているカメラモーダルを検出:', modal.id);
        setupCameraButtons(modal);
      }
    });
  }
  ...
  function setupCameraButtons(modal) {
    ...
    if (videoElement) {
      console.log('カメラビデオ要素を検出:', videoElement.id);
  ...
  function preferNativeCamera() {
    // iOSデバイスではネイティブカメラUIを使用
    if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
      return true;
    }
    
    // 特定のAndroidブラウザでも使用
    if (/Android.*Chrome\/[0-9]/i.test(navigator.userAgent)) {
      return false; // Chromeは内蔵カメラAPIが比較的安定
    }
    
    return false; // デフォルトでは内蔵カメラUIを使用
  }
  ...
  function showCameraModal(targetInput) {
    if (preferNativeCamera()) {
      console.log('ネイティブカメラUIを使用します');
      activateMobileCamera(targetInput);
      return;
    }
    
    console.log('カスタムカメラUIを使用します');
    const modal = createCameraModal();
    document.body.appendChild(modal);
    
    // ターゲット入力要素のIDを保存
    if (targetInput.id) {
      modal.setAttribute('data-target-input', targetInput.id);
    }
  ...
  function createCameraModal() {
    ...
    modal.id = 'mobileCameraModal_' + Date.now();
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">カメラ</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body text-center">
            <div class="camera-container">
              <video id="cameraPreview" autoplay playsinline class="w-100 mb-3"></video>
              <canvas id="cameraCanvas" style="display:none;"></canvas>
            </div>
            <div class="camera-feedback mt-2 alert d-none"></div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">キャンセル</button>
            <button type="button" id="captureButton" class="btn btn-primary">撮影</button>
          </div>
        </div>
      </div>
    `;
